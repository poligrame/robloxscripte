...
local function EnableOrbitTP(enabled)
    ...
    if enabled then
        Connections.OrbitTP = RunService.Heartbeat:Connect(function()
            ...
            -- Increment angles for dome movement
            OrbitAngleH = OrbitAngleH + Settings.OrbitSpeed
            OrbitAngleV = OrbitAngleV + (Settings.OrbitSpeed * 0.7)
            if OrbitAngleH >= 360 then OrbitAngleH = 0 end
            if OrbitAngleV >= 360 then OrbitAngleV = 0 end
            
            -- Calculate dome position (sphere around target)
            local radH = math.rad(OrbitAngleH)
            local radV = math.rad(OrbitAngleV)
            
            local offsetX = math.cos(radH) * math.cos(radV) * Settings.OrbitDistance
            local offsetY = math.abs(math.sin(radV)) * Settings.OrbitDistance
            local offsetZ = math.sin(radH) * math.cos(radV) * Settings.OrbitDistance
            
            local targetPos = targetHRP.Position
            local newPos = Vector3.new(targetPos.X + offsetX, targetPos.Y + offsetY, targetPos.Z + offsetZ)
...
-- Position TP Functions
local function EnablePositionTP(position)
    -- Stop any existing position TP
    if Connections.PositionTP then Connections.PositionTP:Disconnect() Connections.PositionTP = nil end
    
    if not Settings.OrbitTarget then return end
    
    Settings.PositionTP = position
    
    if position == "None" then return end
    
    Connections.PositionTP = RunService.Heartbeat:Connect(function()
        if not Settings.OrbitTarget then return end
        ...
        local dist = Settings.OrbitDistance
        local targetCFrame = targetHRP.CFrame
        local newCFrame
        
        if position == "Behind" then
            newCFrame = targetCFrame * CFrame.new(0, 0, dist)
        elseif position == "Front" then
            newCFrame = targetCFrame * CFrame.new(0, 0, -dist)
        elseif position == "Above" then
            newCFrame = targetCFrame * CFrame.new(0, dist, 0)
        elseif position == "Left" then
            newCFrame = targetCFrame * CFrame.new(-dist, 0, 0)
        elseif position == "Right" then
            newCFrame = targetCFrame * CFrame.new(dist, 0, 0)
        end
        
        if newCFrame then
            myHRP.CFrame = CFrame.new(newCFrame.Position, targetHRP.Position)
        end
    end)
end

local function StopPositionTP()
    Settings.PositionTP = "None"
    if Connections.PositionTP then Connections.PositionTP:Disconnect() Connections.PositionTP = nil end
end
...
local function Init()
    ...
    CreateSlider(UI.Content, "Speed", 1, 100, 30, order, function(v) Settings.OrbitSpeed = v end) order = order + 1
    ...
    -- POSITION TP
    CreateSection(UI.Content, "POSITION TP", order) order = order + 1
    
    local positionLabel = Create("TextLabel", UI.Content, {
        Size = UDim2.new(1, 0, 0, 25),
        BackgroundColor3 = Colors.Secondary,
        BorderSizePixel = 0,
        Text = "Position: None",
    ...
    CreateButton(UI.Content, "TP Behind", order, function()
        if not Settings.OrbitTarget then return end
        StopOrbitTP()
        EnablePositionTP("Behind")
        positionLabel.Text = "Position: Behind"
        positionLabel.TextColor3 = Color3.fromRGB(0, 255, 0)
    end)
    order = order + 1
    
    CreateButton(UI.Content, "TP Front", order, function()
        if not Settings.OrbitTarget then return end
        StopOrbitTP()
        EnablePositionTP("Front")
        positionLabel.Text = "Position: Front"
        positionLabel.TextColor3 = Color3.fromRGB(0, 255, 0)
    end)
    order = order + 1
    
    CreateButton(UI.Content, "TP Above", order, function()
        if not Settings.OrbitTarget then return end
        StopOrbitTP()
        EnablePositionTP("Above")
        positionLabel.Text = "Position: Above"
        positionLabel.TextColor3 = Color3.fromRGB(0, 255, 0)
    end)
    order = order + 1
    
    CreateButton(UI.Content, "TP Left", order, function()
        if not Settings.OrbitTarget then return end
        StopOrbitTP()
        EnablePositionTP("Left")
        positionLabel.Text = "Position: Left"
        positionLabel.TextColor3 = Color3.fromRGB(0, 255, 0)
    end)
    order = order + 1
    
    CreateButton(UI.Content, "TP Right", order, function()
        if not Settings.OrbitTarget then return end
        StopOrbitTP()
        EnablePositionTP("Right")
        positionLabel.Text = "Position: Right"
        positionLabel.TextColor3 = Color3.fromRGB(0, 255, 0)
    end)
    order = order + 1
    
    CreateButton(UI.Content, "Stop Position TP", order, function()
        StopPositionTP()
        positionLabel.Text = "Position: None"
